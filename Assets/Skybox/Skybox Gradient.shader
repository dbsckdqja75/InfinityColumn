...
Shader "Skybox Gradient"
{
	Properties
	{
		...
		_Center("Center", Color) = (0.5,0.5,0.5,0)
	...
	LOD 100
		...
		Pass
		{
			...
			fixed4 frag (v2f i ) : SV_Target
			{
				...
				float4 lerpResult1 = lerp(_Bottom, _Center, pow( saturate( ( staticSwitch13 * _mult ) ) , _pwer ));
				float4 lerpResult2 = lerp(_Center, _Top, pow( saturate( ( staticSwitch13 * _mult ) ) , _pwer ));
				
				float4 lerpResult3 = lerp( lerpResult1 , lerpResult2 , pow( saturate( ( staticSwitch13 * _mult ) ) , _pwer ));
